class AroundMe < ApplicationRecord
  ...
  delegate to: :event, prefix: true # delegate the methods to event, prefix -> prefixes the called method with the parent method name: event-> if lost_dog calls update, it would be prefixed as event_update and event_update would be called from event
